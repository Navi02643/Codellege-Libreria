    ...
    if ((autor.value != '' && titulo.value != '') && (patern.test(autor.value) && patern.test(titulo.value))) {
        if (!LocalStorageOperation.validarLibro(titulo.value.trim().toLowerCase(), autor.value.trim().toLowerCase())) {
            ...
            Swal.fire({
                position: 'center',
                icon: 'success',
                title: 'Se añadio el libro exitosamente',
            ...
            Swal.fire(
                'Libro existente',
                `El libro ya existe en la librería`,
                'error'
            )
        }
    } else {
    ...
    if (event.target.tagName == 'I' || event.target.tagName == 'BUTTON') {
        ...
        if (event.target.className.includes('btn-warning') || event.target.className.includes('fa-trash')) {
            ...
            Swal.fire({
                position: 'center',
                icon: 'success',
                title: 'El libro fue eliminado con exito',
                showConfirmButton: false,
                timer: 1500
            });
        }
...
function buscarLibro(event) {
    ...
    if (inputBuscar.value != '') {
        ...
        if (resultado != '') {
            Swal.fire(
                'Búsqueda exitosa',
                `Titulo: ${resultado.titulo}<br> ID: ${resultado.id}<br> Autor: ${resultado.autor}`,
                'success'
            )
        } else {
            Swal.fire(
